...
@protocol MHHTTPServiceProxy <NSObject>
/**
...
@interface MHHTTPService ()<MHHTTPServiceProxy>
@end
...
#pragma mark -  Parsing
- (void) _parsedResponseOfClass:(Class /*subclass of MHObject*/ )resultClass
...
//        [self _HTTPRequestLog:task body:parameters error:nil];
        ...
        void (^parseJSONDictionary)(NSDictionary *) = ^(NSDictionary *JSONDictionary) {
            ...
            if (resultClass == nil) {
                ...
                /// 1. 先生成MHHTTPResponse对象
                MHHTTPResponse *parsedResponse = [[MHHTTPResponse alloc] initWithResponseObject:responseObject parsedResult:JSONDictionary];
            ...
            if ([JSONArray isKindOfClass:[NSArray class]]) {
                ...
                /// 1. 先生成MHHTTPResponse对象
                MHHTTPResponse *parsedResponse = [[MHHTTPResponse alloc] initWithResponseObject:responseObject parsedResult:parsedObjects];
                /// 2. 回调数据，根据parsedResult 来回调数据
                ...
                /// 1. 先生成MHHTTPResponse对象
                MHHTTPResponse *parsedResponse = [[MHHTTPResponse alloc] initWithResponseObject:responseObject parsedResult:parsedObject];
                /// 2. 回调数据，根据parsedResult 来回调数据
        ...
        if ([responseData isKindOfClass:NSArray.class]) {  /// 数组
            ...
            if (resultClass == nil) {
                /// resultClass == nil 直接把服务器的数据甩回去
                /// 1. 先生成MHHTTPResponse对象
                MHHTTPResponse *parsedResponse = [[MHHTTPResponse alloc] initWithResponseObject:responseObject parsedResult:responseData];
                /// 2. 回调数据，根据parsedResult 来回调数据
            ...
            /// 1. 先生成MHHTTPResponse对象
            MHHTTPResponse *parsedResponse = [[MHHTTPResponse alloc] initWithResponseObject:responseObject parsedResult:nil];
            /// 2. 回调数据，根据parsedResult 来回调数据
    ...
    }else if(statusCode == MHHTTPResponseCodeNotLogin ){
        NSLog(@"+++++ 用户尚未登录或Token失效 ++++")
    }else{
        /// 服务器出错
        NSMutableDictionary *userInfo = [NSMutableDictionary dictionary];
        
        userInfo[MHHTTPServiceErrorHTTPStatusCodeKey] = @(statusCode);
        ...
        userInfo[MHHTTPServiceErrorDescriptionKey] = msgTips;
...
- (void)_parsedErrorWithTask:(NSURLSessionDataTask *)task
            ...
            if ([error.domain isEqual:NSURLErrorDomain]) {
                ...
                switch (error.code) {
                    ...
                    case NSURLErrorClientCertificateRequired:{
                        ...
                        errorDesc = [NSString stringWithFormat:@"呜呜，服务器建立安全连接出错了(%zd)~",MHHTTPServiceErrorSecureConnectionFailed];
                        break;
                    }
    ...
    /// 错误信息
    NSError *parsedError = [NSError errorWithDomain:MHHTTPServiceErrorDomain code:errorCode userInfo:userInfo];
...
- (NSError *)_parsingErrorWithFailureReason:(NSString *)localizedFailureReason {
    ...
    if (localizedFailureReason != nil) userInfo[MHHTTPServiceErrorDescriptionKey] = localizedFailureReason;
