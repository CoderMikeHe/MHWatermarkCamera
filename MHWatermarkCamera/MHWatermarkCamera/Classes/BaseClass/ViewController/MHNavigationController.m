...
- (void)_setupNavigationBarBottomLine{
    ...
    navSystemLine.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:.1];
...
//    [appearance setBarTintColor:[UIColor colorWithRed:rgb green:rgb blue:rgb alpha:0.65]];
    // 设置导航栏的背景渲染色或背景图片
    [appearance setBackgroundImage:[UIImage imageNamed:@"titlebar_bg_7"] forBarMetrics:UIBarMetricsDefault];
    // 设置文字属性
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
    textAttrs[NSFontAttributeName] = MHMediumFont(MHConvertToFitPt(16));
...
+ (void)_setupBarButtonItemTheme{
    ...
    ///
    CGFloat fontSize = MHConvertToFitPt(15);
    
    /**设置文字属性**/
    // 设置普通状态的文字属性
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
    textAttrs[NSForegroundColorAttributeName] = MHColorFromHexString(@"#1ab3ef");
    ...
    highTextAttrs[NSForegroundColorAttributeName] = [MHColorFromHexString(@"#1ab3ef") colorWithAlphaComponent:.5f];
    [appearance setTitleTextAttributes:highTextAttrs forState:UIControlStateHighlighted];
    
    // 设置不可用状态(disable)的文字属性
    NSMutableDictionary *disableTextAttrs = [NSMutableDictionary dictionaryWithDictionary:textAttrs];
    disableTextAttrs[NSForegroundColorAttributeName] = [MHColorFromHexString(@"#1ab3ef") colorWithAlphaComponent:.5f];
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated{
    ...
    if (self.viewControllers.count > 0){
        /// 记录snapView
        MHViewController *topViewController = (MHViewController *)[self topViewController];
        ...
        /// CoderMikeHe Fixed Bug: 隐藏掉系统的返回按钮，这种方法会导致, 侧滑返回取消，导航栏会 莫名显示三个蓝点，及其恶心。但是如果不隐藏返回按钮，则会出现，你在viewController里面设置其 navigationItem.leftBarButtonItem = nil; 就会显示出系统的返回按钮。当然，出现蓝点的解决方案，在MHViewController.m 里面，欢迎大家踊跃讨论
