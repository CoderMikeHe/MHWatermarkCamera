...
@interface MHWebViewController ()<UIScrollViewDelegate>
/// webView
...
- (instancetype)initWithParams:(NSDictionary *)params{
    if (self = [super initWithParams:params]) {
        self.requestUrl = params[MHViewControllerRequestUrlKey];
        _shouldBeginRefreshing = YES;
        _navigationBarHeight = MH_APPLICATION_TOP_BAR_HEIGHT;
        /// 关掉IQKeyboard，以免与H5中控制键盘的逻辑冲突
        self.keyboardEnable = NO;
...
- (void)viewDidLoad {
    ...
    /// 设置子控件
    [self _mh_setupSubViews];
    
    /// 布局子空间
    [self _mh_makeSubViewsConstraints];
...
//    if ((MHIOSVersion >= 9.0)) configuration.applicationNameForUserAgent = userAgent;
    ...
    if(self.shouldPullDownToRefresh){
        [self.webView.scrollView mh_addHeaderRefresh:^(MJRefreshHeader *header) {
            @strongify(self);
            [self.webView reload];
        }];
...
#pragma mark - 布局子控件
- (void)_mh_makeSubViewsConstraints{
