...
NSString *const MHViewControllerIDKey = @"MHViewControllerIDKey";
/// 传递数据模型的key：例如 商品模型的传递 用户模型的传递...
NSString *const MHViewControllerUtilKey = @"MHViewControllerUtilKey";
/// 传递webView Request的key：例如 webView request...
NSString *const MHViewControllerRequestUrlKey = @"MHViewControllerRequestUrlKey";
...
// when `MHViewController ` created and call `viewDidLoad` method , execute `requestRemoteData` Or `configure` method
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    /// 隐藏导航栏细线
    MHNavigationController *nav = (MHNavigationController *)self.navigationController;
    if ([nav isKindOfClass:[MHNavigationController class]]) { /// 容错
        /// 显示或隐藏
...
- (void)viewDidLoad {
    ...
    /// CoderMikeHe Fixed Bug: 适配 iOS 11.0 , iOS11以后，控制器的automaticallyAdjustsScrollViewInsets已经废弃，所以默认就会是YES
    /// iOS 11新增：adjustContentInset 和 contentInsetAdjustmentBehavior 来处理滚动区域
    /// CoderMikeHe Fixed Bug : __IPHONE_11_0 只是说明Xcode定义了这个宏，但不能说明这个支持11.0，所以需要@available(iOS 11.0, *)
#ifdef __IPHONE_11_0
