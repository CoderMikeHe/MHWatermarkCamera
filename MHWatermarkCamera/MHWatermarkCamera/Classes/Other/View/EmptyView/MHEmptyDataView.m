...
#import "MHEmptyDataView.h"

@interface MHEmptyDataView ()
...
@implementation MHEmptyDataView
...
- (void)configEmptyViewWithType:(MHEmptyDataViewType)type emptyInfo:(NSString *)emptyInfo errorInfo:(NSString *)errorInfo offsetTop:(CGFloat)offsetTop hasData:(BOOL)hasData hasError:(BOOL)hasError reloadBlock:(void(^)(void))reloadBlock{
    ...
    NSInteger random = [NSObject mh_randomNumberWithFrom:0 to:1];
    UIImage *image = nil;
    if (hasError) { /// 请求出错 1. 网络问题  2. 服务器问题
        if (![MHHTTPService sharedInstance].reachabilityManager.isReachable) {  /// 无网络
            errorInfo = MHStringIsNotEmpty(errorInfo)?errorInfo:@"呀！网络正在开小差~";
            image = [UIImage imageNamed:[NSString stringWithFormat:@"mh_default_no_wifi_%ld",(long)random]];
        }else{    /// 服务器出错
            errorInfo = MHStringIsNotEmpty(errorInfo)?errorInfo:@"呜呜！服务器崩溃了~";
            image = [UIImage imageNamed:@"mh_default_service_error"];
        }
    ...
    /// 无数据
    switch (type) {
        case MHEmptyDataViewTypeDefault:  /// 默认情况
            {
                emptyInfo = MHStringIsNotEmpty(emptyInfo)?emptyInfo:@"哇喔！空空如也~";
                image = [UIImage imageNamed:[NSString stringWithFormat:@"mh_default_empty_data_%ld",(long)random]];
            }
            break;
        case MHEmptyDataViewTypeBuyGoods:
        {
            emptyInfo = MHStringIsNotEmpty(emptyInfo)?emptyInfo:@"没买到心仪的宝贝\n去首页逛逛吧～";
            image = [UIImage imageNamed:@"mh_default_buy_goods"];
        }
            break;
        case MHEmptyDataViewTypeSoldGoods:
        {
            emptyInfo = MHStringIsNotEmpty(emptyInfo)?emptyInfo:@"你的商品无人问津\n分享到朋友圈吆喝一下～";
            image = [UIImage imageNamed:@"mh_default_sold_goods"];
        }
            break;
        case MHEmptyDataViewTypePublishGoods:
        {
            emptyInfo = MHStringIsNotEmpty(emptyInfo)?emptyInfo:@"赶紧找出家里的闲置上传吧\n发布出来可以换钱哦～";
            image = [UIImage imageNamed:@"mh_default_publish_goods"];
        }
            break;
        case MHEmptyDataViewTypeCollectGoods:
        {
            emptyInfo = MHStringIsNotEmpty(emptyInfo)?emptyInfo:@"你还没有收藏相关商品\n赶快去逛逛吧～";
            image = [UIImage imageNamed:@"mh_default_collect_goods"];
        }
            break;
        case MHEmptyDataViewTypeGoodsDetail:
        {
            emptyInfo = MHStringIsNotEmpty(emptyInfo)?emptyInfo:@"这个商品不见了\n换一个商品再看看～";
            image = [UIImage imageNamed:@"mh_default_goods_detail"];
        }
            break;
        case MHEmptyDataViewTypeOrderDetail:
        {
            emptyInfo = MHStringIsNotEmpty(emptyInfo)?emptyInfo:@"暂无订单消息，去首页逛逛吧～";
            image = [UIImage imageNamed:@"mh_default_order_detail"];
        }
            break;
        case MHEmptyDataViewTypeSearchGoods:
        {
            emptyInfo = MHStringIsNotEmpty(emptyInfo)?emptyInfo:@"这是一道超岗题\n换一个关键词看看～";
            image = [UIImage imageNamed:@"mh_default_search_no_result"];
        }
            break;
        case MHEmptyDataViewTypeAddress:
        {
            emptyInfo = MHStringIsNotEmpty(emptyInfo)?emptyInfo:@"添加地址信息，下单更快捷～";
            image = [UIImage imageNamed:@"mh_default_my_address"];
        }
            break;
        case MHEmptyDataViewTypeRedPocket:
        {
            emptyInfo = MHStringIsNotEmpty(emptyInfo)?emptyInfo:@"您还没有可用的红包哦～";
            image = [UIImage imageNamed:@"mh_default_red_pocket"];
        }
            break;
...
- (void)_setupSubViews{
    ...
    [reloadButton setTitleColor:MH_MAIN_TINTCOLOR forState:UIControlStateNormal];
    [reloadButton setTitle:@"重新连接" forState:UIControlStateNormal];
    reloadButton.layer.cornerRadius = 4.f;
    reloadButton.layer.borderColor = MH_MAIN_TINTCOLOR.CGColor;
